...
balance = Number(window.localStorage.getItem("balance")) || 500000;
...
const render = () => {
    ...
    transactions.forEach((e, i) => {
      transactionList.innerHTML = `
        <li class="transaction ${e.type}">
          <p>${e.name}</p>
          <div class="right">
            <p>${currencySymbol}${e.amount.toLocaleString()}</p>
    ...
    currentBalancePlaceholder.innerHTML = `${balance.toLocaleString()}`
